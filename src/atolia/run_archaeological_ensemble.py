...
def run_one(hypothesis: Mapping[str, Any], seed: int, steps: int, candidates: int, catalogue: int, workshops: int) -> Dict[str, Any]:
    ...
    # Round 3/4 must use the same actual source/workshop/guild substrate as the
    # object-level game. The former graph-only shortcut erased exactly that information.
    ...
    return {
        ...
        "physical_truth": {
            "workshops": len(world.workshops), "sources": len(world.sources),
            "guilds": len(getattr(world, "guilds", {})),
            "geography": getattr(world, "geography_report", {}),
        },
    }
...
def summarize_ensemble(runs: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    def stats(vals: Sequence[float]) -> Dict[str, float]:
        ...
        return {"mean": float(a.mean()), "sd": float(a.std(ddof=0)), "p05": float(np.quantile(a, .05)),
                "p50": float(np.quantile(a, .50)), "p95": float(np.quantile(a, .95)),
