...
import physical_condensation as condensation
...
def _flatten_distribution(report: Mapping[str, Any]) -> Dict[str, float]:
    ...
    for axis, values in dist.items():
        for key, value in values.items(): out[f"{axis}/{key}"] = float(value)
    return out


def _family_metrics(report: Mapping[str, Any]) -> Dict[str, float]:
    dist = report["stages"]["catalogue_30k"]["distribution"]
    dep = dist.get("deposition_region", {}); distance = dist.get("distance_bin", {})
    phys = dist.get("physical_crossings", {}); field = dist.get("field_crossings", {}); obj = dist.get("object_class", {})
    return {
...
def run_one(hypothesis: Mapping[str, Any], seed: int, steps: int, candidates: int, catalogue: int, workshops: int) -> Dict[str, Any]:
    world = afw.FieldArchaeologicalObservationWorld(hypothesis, seed=seed)
    ...
    return {"seed": seed, "flow": flow_summary, "materializer": materializer_summary,
            "catalogue": catalogue_summary, "waterfall": waterfall,
            "family_metrics": _family_metrics(waterfall),
            "physical_truth": {"workshops": len(world.workshops), "sources": len(world.sources),
                               "guilds": len(getattr(world, "guilds", {})), "geography": getattr(world, "geography_report", {})}}


def summarize_ensemble(runs: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    metrics: Dict[str, List[float]] = defaultdict(list); distribution_values: Dict[str, List[float]] = defaultdict(list)
    for run in runs:
        for k, v in run["family_metrics"].items(): metrics[k].append(float(v))
        for k, v in _flatten_distribution(run["waterfall"]).items(): distribution_values[k].append(float(v))
    def stats(vals: Sequence[float]) -> Dict[str, float]:
        a = np.asarray(vals, dtype=float)
        return {"mean": float(a.mean()), "sd": float(a.std(ddof=0)), "p05": float(np.quantile(a,.05)),
                "p50": float(np.quantile(a,.50)), "p95": float(np.quantile(a,.95)), "min": float(a.min()), "max": float(a.max())}
    return {"ensemble_size": len(runs), "seed_range": [int(runs[0]["seed"]), int(runs[-1]["seed"])] if runs else [],
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run deterministic Round-4 archaeology ensembles with persistent physical finds")
    ap.add_argument("--hypothesis", type=Path, default=Path("hypotheses/atolia_atesis_1800_1000_v0.json"))
    ap.add_argument("--seed-start", type=int, default=1); ap.add_argument("--runs", type=int, default=20)
    ap.add_argument("--steps", type=int, default=28); ap.add_argument("--candidates", type=int, default=100_000)
    ap.add_argument("--catalogue", type=int, default=30_000); ap.add_argument("--workshops", type=int, default=3200)
    ap.add_argument("--out", type=Path, default=Path("output/archaeological_ensemble.json")); args = ap.parse_args()
    hypothesis = json.loads(args.hypothesis.read_text(encoding="utf-8")); runs=[]
    for seed in range(args.seed_start, args.seed_start + args.runs):
        print(f"round4 seed {seed} ...", file=sys.stderr, flush=True)
        runs.append(run_one(hypothesis, seed, args.steps, args.candidates, args.catalogue, args.workshops))
    payload={"model_version": condensation.CONDENSATION_VERSION,
             "parameters": {"steps":args.steps,"candidates":args.candidates,"catalogue":args.catalogue,"workshops":args.workshops},
             "ensemble": summarize_ensemble(runs), "runs":runs}
